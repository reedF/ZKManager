...
public class ZkCfgManagerImpl implements ZkCfgManager {
	...
	// JdbcConnectionPool.create("jdbc:h2:tcp://192.168.10.84:19200/~/zkcfg","h2","h2");
	private static JdbcConnectionPool cp = JdbcConnectionPool.create(h2con, h2user, h2password);

	// private static JdbcConnectionPool cp =
	// JdbcConnectionPool.create("jdbc:h2:tcp://127.0.0.1/~/zkcfg","sa","sa");
	...
	public boolean init() {
		...
		try {
			...
			e.printStackTrace();
			log.error("init zkCfg error : {}", e.getMessage());
		} finally {
			if (null != ps) {
				try {
					ps.close();
